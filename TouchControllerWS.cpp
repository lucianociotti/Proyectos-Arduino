...
void TouchControllerWS::continueCalibration() {
    if (!touchScreen->touched()) {
        Serial.println("Pantalla no tocada.");
        return;
    }

    TS_Point p = touchScreen->getPoint();

    // Imprimir los valores de p.x y p.y para depuración
    Serial.print("Touch point detected at ");
    Serial.print(p.x);
    Serial.print("/");
    Serial.println(p.y);

    // Verificar si el punto detectado es válido
    if (p.x == -1 || p.y == -1) {
        Serial.println("Punto inválido detectado, ignorando...");
        return; // Salir de la función si el punto es inválido
    }
